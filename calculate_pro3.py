                ...
                for sl in [1.0, 1.5, 2.0]:
                    ...
                    for i in range(len(test_df)):
                        ...
                        if res != "NO_DATA":
                            ...
                            total_r += r_score
                            next_entry = exit_t
                            trade_logs.append({"通貨": tk, "種別": direction, "Entry時間 (JST)": curr_time.astimezone(jst).strftime('%m/%d %H:%M'), "Exit時間 (JST)": exit_t.astimezone(jst).strftime('%m/%d %H:%M'), "Entry価格": f"{entry_p:.5f}", "Exit価格": f"{exit_p:.5f}", "結果": res, "Rスコア": r_score})
                            u = 10000 / (sl_dist * (usdjpy_rate if "USD" in tk else 1.0)) if sl_dist > 0 else 0
                            total_units += u
                    
                    if total_r > best_r:
                        best_r = total_r
                        best_combo = {"asset": tk, "rr": rr, "sl": sl, "steps": h, "r_profit": total_r, "trades": len(trade_logs), "logs": trade_logs, "avg_u": total_units / (len(trade_logs) or 1)}
    return best_combo
...
def display_best_result(best):
    ...
    st.sidebar.success(f"**【最強設定が判明しました】**\n\n👑 通貨ペア: **{best['asset']}**\n\n⏳ 使用時間足: **{tf_choice}** (予測本数: {best['steps']}本)\n\n⚖️ RR比率: **{best['rr']}**\n\n🛑 損切り(ATR): **{best['sl']}**\n\n📊 平均取引量: **約{best['avg_u']:,.2f} 通貨**\n\n💰 利益: **+{est_profit_jpy:,.0f}円**\n\n※回数: {best['trades']}回")
    ...
    with st.spinner("解析中..."):
        ...
        if df is not None:
            ...
            if res:
                ...
                st.info(f"🧭 相場環境: **{'トレンド' if adx > 25 else 'レンジ'}** (ADX: {adx:.1f}) | 足確定: {used_t.astimezone(jst)}")
                col1, col2 = st.columns(2)
                col1.metric("開始価格", f"{price_now:.5f}"); col2.metric("AI予測", "UP ↗️" if proba[1]>proba[0] else "DOWN ↘️", f"確信度: {max(proba)*100:.1f}%")
                sl_dist = atr * sl_input; tp_dist = sl_dist * rr_input
                ...
                c_tp, c_ent, c_sl = st.columns(3); c_tp.warning(f"利確: {tp_p:.5f}"); c_ent.info(f"Entry: {price_now:.5f}"); c_sl.error(f"損切り: {sl_p:.5f}")
                st.line_chart(df.tail(50)['Close']); st.subheader("🧠 AIの判断根拠"); st.bar_chart(fi.set_index('Feature'))

if 'best_logs' in st.session_state:
    st.markdown("---"); st.subheader("📑 バックテスト詳細履歴 (最強設定のログ)")
