        ...
        for rr in [1.5, 2.0, 3.0]:
            for sl in [1.0, 1.5, 2.0]:
                ...
                if total_r > best_r:
                    best_r = total_r
                    best_combo = {
                        "asset": tk, "rr": rr, "sl": sl, "r_profit": total_r,
...
if st.sidebar.button("🏆 最強設定を自動探索"):
    with st.spinner("全通貨・全設定を総当たり検証中..."):
        ...
        if best:
            st.sidebar.success(f"👑 **{best['asset']}** ({tf_choice})\n\n"
                               f"⚖️ RR: {best['rr']} / 🛑 SL: {best['sl']}ATR\n"
                               f"📊 平均量: {best['avg_u']:,.0f} Units\n"
                               f"💰 利益: +{best['r_profit']*10000:,.0f}円")
